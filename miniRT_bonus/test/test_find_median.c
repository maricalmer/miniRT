...
/*   Updated: 2025/04/06 18:50:46 by dlemaire         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	test_find_median_custom_odd(void)
{
	t_bvh	*bvh = create_mock_bvh(5);
	int		idx = 0;
	int		axis = 0;
	float	median = find_median_custom(bvh, idx, axis);

	CU_ASSERT_TRUE(assert_float_equal(median, 2.0, EPSILON_TEST));
}

void	test_find_median_custom_even(void)
{
...
int	add_find_median_tests(void)
{
	...
	if (!suite)
		return (CU_get_error());
	if (!CU_add_test(suite, "test_find_median_custom_odd", test_find_median_custom_odd)
		|| !CU_add_test(suite, "test_find_median_custom_even", test_find_median_custom_even))
	{
		return (CU_get_error());
	}
