...
/*   Updated: 2025/04/26 15:13:24 by dlemaire         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
float	intersect_rectangle(t_object *obj, float ray[3], float origin[3])
{
	...
	if (a < EPSILON && a > -EPSILON)
		return (0);
	vec_substr(obj->geo.rec.point, origin, delta_pos);
	b = dot_13_13(obj->geo.rec.normal, delta_pos);
	if ((a < 0 && b > 0) || (a > 0 && b < 0))
		return (0);
	a = b / a;
	...
	vec_substr(hit_pt, obj->geo.rec.point, delta_pos);
	uv[0] = dot_13_13(delta_pos, obj->geo.rec.u);
	uv[1] = dot_13_13(delta_pos, obj->geo.rec.v);
